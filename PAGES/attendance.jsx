...
export default function AttendancePage() {
  ...
  useEffect(() => {
    ...
    init();
    const onChanged = () => {
      // refresh courses list; student/attendance data reload already tied to selectedCourseId
      (async () => {
        try {
          const currentUser = await User.me();
          let userCourses = [];
          if (currentUser.role === 'admin') userCourses = await Course.list();
          else {
            const facultyProfile = await Faculty.filter({ email: currentUser.email });
            if (facultyProfile.length > 0) userCourses = await Course.filter({ faculty_id: facultyProfile[0].id });
          }
          setCourses(userCourses);
        } catch (_e) {}
      })();
  ...
  const fetchStudentsForCourse = useCallback(async (courseId) => {
    ...
    try {
      // Get all enrollments for this course with status 'Enrolled'
      ...
      if (enrollments.length > 0) {
        ...
        // Filter students that are enrolled in this course
  ...
  }, [selectedCourseId, fetchStudentsForCourse]);

  // Listen for enrollment changes to refresh student list
  useEffect(() => {
    const onEnrollmentChanged = (e) => {
      if (!e.detail || e.detail.name === 'Enrollment' || e.detail.name === 'Student') {
        if (selectedCourseId) {
          fetchStudentsForCourse(selectedCourseId);
        }
      }
    };
    window.addEventListener('sms:entity-changed', onEnrollmentChanged);
    return () => window.removeEventListener('sms:entity-changed', onEnrollmentChanged);
  }, [selectedCourseId, fetchStudentsForCourse]);
