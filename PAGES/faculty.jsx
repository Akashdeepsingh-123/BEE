...
export default function FacultyPage() {
  ...
  useEffect(() => {
    loadFaculty();
    const onChanged = (e) => {
      if (!e.detail || e.detail.name === 'Faculty') loadFaculty();
    };
  ...
  const handleSubmit = async (facultyData) => {
    try {
      let savedFaculty;
      if (editingFaculty) {
        savedFaculty = await Faculty.update(editingFaculty.id, facultyData);
      } else {
        savedFaculty = await Faculty.create(facultyData);
      }

      // Sync user account for faculty login
      if (savedFaculty && savedFaculty.email) {
        try {
          await User.syncFacultyAccount(savedFaculty, { sendEmail: true });
        } catch (syncError) {
          console.error('Error syncing faculty account:', syncError);
          // Don't block the form submission if sync fails
        }
      }
