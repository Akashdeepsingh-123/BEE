...
export default function StudentsPage() {
  ...
  useEffect(() => {
    loadStudents();
    const onChanged = (e) => {
      if (!e.detail || e.detail.name === 'Student') loadStudents();
    };
  ...
  const handleSubmit = async (studentData) => {
    try {
      if (editingStudent) {
        ...
        const updated = await Student.update(editingStudent.id, studentData);
        const emailChanged = editingStudent.email !== studentData.email;
        await User.syncStudentAccount(updated, { sendEmail: emailChanged });
      } else {
        ...
        await User.syncStudentAccount(newStudent, { sendEmail: true });
      }
      setShowForm(false);
